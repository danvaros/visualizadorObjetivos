! function() {
    var a = function() {
        return function() {
            this.series = void 0, this.numClasses = null, this.breaks = void 0, this.colorCode = void 0, this.range = void 0, this.statMethod = void 0, this.colorSchemes = {
                OrRd: {
            ...
            }, this.setSeries = function(a) {
                this.series = Array(), this.series = a, this.series = this.series.sort(function(a, b) {
                    return a - b
                })
            }, this.getSeries = function() {
                return this.series
            }, this.setNumClasses = function(a) {
                this.numClasses = a
            }, this.getNumClasses = function() {
                return this.numClasses
            }, this.setColorCode = function(a) {
                this.colorCode = a
            }, this.getColorCode = function() {
                return this.colorCode
            }, this.getColorCodes = function() {
                var a = [];
                for (code in this.colorSchemes) this.colorSchemes.hasOwnProperty(code) && a.push(code);
                return a
            }, this.getColorCodesByType = function() {
                var a = {};
                for (code in this.colorSchemes) this.colorSchemes.hasOwnProperty(code) && (a.hasOwnProperty(this.colorSchemes[code].properties.type) || (a[this.colorSchemes[code].properties.type] = []), a[this.colorSchemes[code].properties.type].push(code));
                return a
            }, this._classifyEqualInterval = function() {
                var a = Math.min.apply(null, this.series),
                    b = Math.max.apply(null, this.series),
                    c = [],
                    d = a,
                    e = (b - a) / this.getNumClasses();
                for (i = 0; i <= this.getNumClasses(); i++) c[i] = d, d += e;
                return c[this.getNumClasses()] = b, this.range = c, this.range.sort(function(a, b) {
                    return a - b
                }), this.range
            }, this._classifyQuantile = function() {
                for (var a = this.series.sort(function(a, b) {
                        return a - b
                    }), b = [], c = this.series.length / this.getNumClasses(), d = 1; d < this.getNumClasses(); d++) {
                    var e = Math.round(d * c + .49);
                    b.push(a[e - 1])
                }
                var f = b;
                return f.unshift(a[0]), f[a.length - 1] !== a[a.length - 1] && f.push(a[a.length - 1]), this.range = f, this.range.sort(function(a, b) {
                    return a - b
                }), this.range
                ...
                if (this.getNumClasses % 2 == 1) {
                    var d = Math.floor(this.getNumClasses() / 2),
                        e = d + 1;
                    for (c[d] = this._mean(this.series) - this._stdDev(this.series) / 2, c[e] = this._mean(this.series) + this._stdDev(this.series) / 2, i = d - 1; i > 0; i--) {
                        var f = c[i + 1] - this._stdDev(this.series);
                        c[i] = f
                    }
                    for (i = e + 1; i < this.getNumClasses(); i++) {
                        var f = c[i - 1] + this._stdDev(this.series);
                        c[i] = f
                    }
                } else {
                    var g = this.getNumClasses() / 2;
                    for (c[g] = this._mean(this.series), i = g - 1; i > 0; i--) {
                        var f = c[i + 1] - this._stdDev(this.series);
                        c[i] = f
                    }
                    for (i = g + 1; i < this.getNumClasses(); i++) {
                        var f = c[i - 1] + this._stdDev(this.series);
                        c[i] = f
                    }
                }
                return c[0] = a, c[this.getNumClasses()] = b, this.range = c, this.range.sort(function(a, b) {
                    return a - b
                }), this.range
            }, this._classifyJenks = function() {
                for (var a = [], b = 0, c = this.series.length + 1; c > b; b++) {
                    for (var d = [], e = 0, f = this.numClasses + 1; f > e; e++) d.push(0);
                    a.push(d)
                }
                for (var g = [], h = 0, i = this.series.length + 1; i > h; h++) {
                    for (var j = [], k = 0, l = this.numClasses + 1; l > k; k++) j.push(0);
                    g.push(j)
                }
                for (var m = 1, n = this.numClasses + 1; n > m; m++) {
                    a[0][m] = 1, g[0][m] = 0;
                    for (var o = 1, p = this.series.length + 1; p > o; o++) g[o][m] = 1 / 0;
                    var q = 0
                }
                for (var r = 2, s = this.series.length + 1; s > r; r++) {
                    for (var t = 0, u = 0, v = 0, w = 1, x = r + 1; x > w; w++) {
                        var y = r - w + 1,
                            z = parseFloat(this.series[y - 1]);
                        u += z * z, t += z, v += 1, q = u - t * t / v;
                        var A = y - 1;
                        if (0 != A)
                            for (var B = 2, C = this.numClasses + 1; C > B; B++) g[r][B] >= q + g[A][B - 1] && (a[r][B] = y, g[r][B] = q + g[A][B - 1])
                    }
                    a[r][1] = 1, g[r][1] = q
                }
                var D = this.series.length,
                    E = [];
                for (h = 0, i = this.numClasses + 1; i > h; h++) E.push(0);
                E[this.numClasses] = parseFloat(this.series[this.series.length - 1]), E[0] = parseFloat(this.series[0]);

                for (var F = this.numClasses; F >= 2;) {
                    if((a[D][F] - 2) <= 0)
                    var G = parseInt(a[D][F] - 2);
                    E[F - 1] = this.series[G], D = parseInt(a[D][F] - 1), F -= 1
                }

                return E[0] == E[1] && (E[0] = 0), this.range = E, this.range.sort(function(a, b) {
                    return a - b
                }), this.range
            }, this.classify = function(a, b) {
                this.statMethod = void 0 !== a ? a : this.statMethod, this.numClasses = void 0 !== b ? b : this.numClasses;
                var c = void 0;
                switch (a) {
                    case "equal_interval":
                        c = this._classifyEqualInterval();
                        break;
                    case "quantile":
                        c = this._classifyQuantile();
                        break;
                    case "std_deviation":
                        c = this._classifyStdDeviation();
                        break;
                    case "jenks":
                        c = this._classifyJenks();
                        break;
                    default:
                        c = this._classifyJenks()
                }
                return this.breaks = c, c
            }, this.getClassificationMethods = function() {
                return ["equal_interval", "quantile", "jenks"]
            ...
            }, this.getColorInRange = function(a) {
                var b = 0;
                for (b; b < this.range.length; b++) {
                    if (!(a >= this.range[b] && this.range[b + 1])) return a == this.range[b] ? this.colorSchemes[this.colorCode][this.numClasses][b - 1] : !1;
                    if (a < this.range[b + 1]) return this.colorSchemes[this.colorCode][this.numClasses][b]
                }
            }, this._mean = function(a) {
                return parseFloat(this._sum(a) / a.length)
            }, this._sum = function(a) {
                var b, c = 0;
                for (b = 0; b < a.length; b++) c += a[b];
                return c
            }, this._variance = function(a) {
                for (var b = 0, c = 0; c < a.length; c++) b += Math.pow(a[c] - this._mean(a), 2);
                return b / a.length
            }, this._stdDev = function(a) {
                return Math.sqrt(this._variance(a))
            }
        }
    }();
    "undefined" != typeof module && "undefined" != typeof module.exports ? module.exports = a : window.classyBrew = a
}();
